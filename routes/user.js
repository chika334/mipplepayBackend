...
router.get('/auth/me', (req, res) => {
	...
	jwt.verify(token, process.env.JWT_SECRET, function(err, decoded) {
		...
		User.findById(decoded._id, function(err, user) {
			...
			res.status(200).send({ user: { _id, firstName, lastName, email, role } });
		});
	});
});
...
router.post('/register', async (req, res) => {
	...
	// check if all inputs are filled
	if (firstName === '' || lastName === '' || email === '' || phone === '' || countryCode === '' || password === '') {
		return res.status(400).json({
			msg: 'Input all fields'
		});
	}
	...
	const token = user.generateAuthToken();
	const { _id, role } = user;
	res.header('x-auth-token', token).send({ user: { _id, firstName, lastName, email, phone, countryCode, role }, token });
});
